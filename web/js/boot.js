...
(function () {
  ...
  function apply() {
    ...
    try {
      if (page === "index") {
        var h = (HAO.getHero() || [])[0];
        if (h) {
          var egEl = document.querySelector(".hero2__eg");
          var tEl = document.querySelector(".hero2__t");
          var dEl = document.querySelector(".hero2__d");
          var mEl = document.querySelector(".hero2__media");
          if (egEl && h.eng != null) egEl.textContent = h.eng;
          if (tEl && h.title != null) tEl.innerHTML = HAO.fmt(h.title, "em");
          if (dEl && h.desc != null) dEl.textContent = h.desc;
          if (mEl && h.img) mEl.setAttribute("src", HAO.imgSrc(h.img));
        }
      }
    } catch (e) {}
    ...
    try {
      ...
      if (p) {
        ...
        if (base) {
          var url = base + "/web/" + (page === "index" ? "" : file);
    ...
    try {
      if (!document.getElementById("hwHeadCode")) {
        ...
        if (code && code.trim()) {
          ...
          [].slice.call(wrap.childNodes).forEach(function (node) {
            if (node.tagName === "SCRIPT") {
              ...
              s.text = node.text; s.id = s.id || "hwHeadCode"; document.head.appendChild(s);
            } else if (node.nodeType === 1) {
              document.head.appendChild(node.cloneNode(true));
            }
          });
          var flag = document.createElement("meta"); flag.id = "hwHeadCode"; document.head.appendChild(flag);
        }
      }
    } catch (e) {}
    ...
    try {
      if (/[?&]edit=1/.test(location.search) && localStorage.getItem("hw_edit") === "1") startEditMode("hw_copy");
    } catch (e) {}
  }
  ...
  function startEditMode(copyKey) {
    ...
    HAO.getCopy().forEach(function (c) {
      if (c.page !== "all" && c.page !== page) return;
      if (c.attr) return; /* 숫자/속성 카피는 관리자 폼에서 수정 */
