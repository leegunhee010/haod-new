// 하오 마케팅센터 — 히어로 홀로그래픽 글래스 (Three.js 실제 굴절·분산)
...
function initHolo(canvas) {
  ...
  // ── 홀로그래픽 환경맵 (유리가 굴절·반사할 색) : 무지개 전체 + 오렌지 우세
  ...
  const warm = new THREE.PointLight(0xff7a2e, 70, 40); warm.position.set(-5, -2, 3); scene.add(warm);
  const cool = new THREE.PointLight(0xff52d6, 40, 40); cool.position.set(5, 3, 2); scene.add(cool);
  scene.add(new THREE.AmbientLight(0xffffff, 0.5));

  function glassMat() {
    const m = new THREE.MeshPhysicalMaterial({
      transmission: 0.72, thickness: 0.7, roughness: 0.02, metalness: 0,
      ior: 1.5, clearcoat: 1, clearcoatRoughness: 0.02,
      iridescence: 1, iridescenceIOR: 1.5, iridescenceThicknessRange: [140, 1150],
      attenuationColor: new THREE.Color(0xffe2cc), attenuationDistance: 6,
      envMapIntensity: 3.5, transparent: true
    });
    if ('dispersion' in m) m.dispersion = 22; // r163+ 색분산(무지개 가장자리)
    return m;
  }
  ...
  // 링 / 캡슐 / 다면체 블롭 / 작은 링 — HAO 워드마크 위를 가로질러 배치
  add(new THREE.TorusGeometry(1.0, 0.4, 96, 240), [-0.6, 0.25, 0], 1.05, [0.5, 0.3, 0], [0.18, 0.26, 0.05]);
  add(new THREE.CapsuleGeometry(0.42, 1.15, 56, 112), [0.95, 0.45, -0.4], 1.0, [0.7, 0.2, 0.9], [0.12, 0.3, 0.1]);
  add(new THREE.SphereGeometry(0.82, 96, 96), [2.6, -0.25, 0.3], 1.0, [0.2, 0.3, 0], [0.04, 0.1, 0.03]);
  add(new THREE.TorusGeometry(0.62, 0.27, 80, 180), [3.55, 0.6, -0.3], 0.85, [1.2, 0.6, 0.3], [0.24, 0.18, 0.2]);

  // 환경 텍스처: 홀로그래픽 그라데이션 + 하이라이트 블롭
  function makeEnvTexture() {
    const c = document.createElement('canvas'); c.width = 1024; c.height = 512;
    const x = c.getContext('2d');
    // 진한 베이스 + 채도 높은 무지개 띠 (오렌지 우세) — 반사·굴절에 색이 진하게 실리도록
    const g = x.createLinearGradient(0, 0, 1024, 512);
    g.addColorStop(0.00, '#0a0610');
    g.addColorStop(0.15, '#ff5a00'); // 진오렌지 (우세)
    g.addColorStop(0.30, '#ff9a2e');
    g.addColorStop(0.44, '#ffd000'); // 골드
    g.addColorStop(0.58, '#ff5a7a'); // 코랄(핑크 완화)
    g.addColorStop(0.70, '#9a6cff'); // 바이올렛
    g.addColorStop(0.83, '#00b3ff'); // 시안
    g.addColorStop(0.93, '#46ffe0'); // 민트
    g.addColorStop(1.00, '#ffffff');
    x.fillStyle = g; x.fillRect(0, 0, 1024, 512);
    // 색 띠 사이 어두운 분리선으로 대비 강화
    x.globalAlpha = 0.55;
    ...
    // 날카로운 스페큘러 소프트박스(흰 둥근 사각형) — 유리에 또렷한 글린트
    ...
    softbox(170, 120, 120, 40);
    softbox(620, 90, 50, 180);
    softbox(340, 400, 90, 30);
    softbox(880, 300, 40, 120);
  ...
  function resize() {
    ...
    // 좁은 화면에선 도형을 가운데로 모으고 축소
    const narrow = w < 900;
    group.scale.setScalar(narrow ? 0.7 : 1);
    group.position.x = narrow ? -1.2 : 0;
  }
  ...
  function frame() {
    ...
    for (let i = 0; i < shapes.length; i++) {
      ...
      // 큰 부유 + 가로 드리프트
      const fy = Math.sin(t * 1.15 + s.ph) * 0.30;
      const fx = Math.cos(t * 0.85 + s.ph * 0.7) * 0.18;
      // 마우스 깊이 패럴랙스 — 앞쪽(depth 큰) 도형이 더 많이 따라옴
      s.mesh.position.x = s.baseX + fx + mxS * s.depth * 2.6;
      s.mesh.position.y = s.baseY + fy - myS * s.depth * 1.9;
      s.mesh.position.z = s.baseZ + mxS * s.depth * 0.7;
    }
    // 그룹 회전(마우스 강하게) + 전체 미세 흔들림
    group.rotation.y += (mxS * 0.7 - group.rotation.y) * 0.06;
    group.rotation.x += (-myS * 0.45 - group.rotation.x) * 0.06;
    group.rotation.z = Math.sin(t * 0.4) * 0.04;
    // 카메라 패럴랙스 (장면 전체가 시점따라 미끄러짐)
