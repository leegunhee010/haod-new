...
**(b) 카페24/Cloudflare 자동 변환** — 카페24 호스팅에 Cloudflare(무료 플랜의 Polish 또는 이미지 리사이징)를 얹으면 서버가 방문자 브라우저에 맞춰 WebP/AVIF를 자동 제공. 코드 수정 없이 적용 가능 → **가장 손쉬운 방법**.

**(c) Squoosh.app** — 소수 핵심 이미지(히어로 등)만 수동 변환.
